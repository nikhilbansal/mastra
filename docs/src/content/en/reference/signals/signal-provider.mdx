...
Observe the authoritative start and finish boundaries of an agent run. The Agent runs its configured lifecycle callback first, then calls each provider observer. Provider errors are logged and don't change the run outcome.

```typescript prettier:false
import type { AgentRunLifecycleEvent } from '@mastra/core/agent'

async onRunLifecycle(event: AgentRunLifecycleEvent) {
  if (event.phase !== 'finish' || event.outcome === 'suspended') return
  await this.clearCompletedRunState(event.threadId)
}
```
...
```typescript prettier:false
const sub = this.subscribe({ threadId: 'thread-1', resourceId: 'user-1' }, 'github:mastra-ai/mastra#123', { pr: 123 });
```
...
```typescript prettier:false
const removed = this.unsubscribe({ threadId: 'thread-1', resourceId: 'user-1' }, 'github:mastra-ai/mastra#123');
```
