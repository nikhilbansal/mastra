...
When a run finishes successfully, fails, or is canceled, the provider clears the task list and persists an empty state snapshot. A suspended run keeps its exact list so a later human-in-the-loop resume can continue it.
