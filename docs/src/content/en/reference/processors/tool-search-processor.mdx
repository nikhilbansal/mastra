    ...
    properties: [
      {
        ...
        parameters: [
          ...
          },
          {
            ...
            description:
              "Where loaded-tool state lives. 'in-memory' (default) tracks loaded tools in an in-memory map per thread with TTL cleanup (see ttl); state is lost on restart and anonymous requests share a 'default' entry. 'context' derives loaded state from the conversation messages — a tool is loaded while a completed direct invocation or a search_tools/load_tool result naming it remains in the messages; it is restart-safe, requires no memory, and de-loads automatically once those records are no longer present in the messages. The 'context' store is opt-in.",
            isOptional: true,
            default: "'in-memory'",
          },
          {
...
Loaded state is derived from the conversation messages: a tool is loaded while a completed direct invocation, or a `search_tools` or `load_tool` result naming it, remains in the messages. This mode:

- Requires no memory configuration.
- Is restart-safe: the durable record is the persisted message history.
- De-loads a tool automatically once neither record is present in the messages.
...
Unloading a tool changes the tool definitions sent to the model, which shifts the cached prefix and causes the next turn to pay a cache write instead of a cache hit. In `'in-memory'` mode this happens when a thread's state is evicted by `ttl`. In `'context'` mode it happens when neither a discovery result nor a completed direct invocation for the tool remains in the messages (for example, when older messages are trimmed). The tool de-loads, and the model must search for it again before reuse. This is expected: removing an unused tool trades one cache write for a smaller prefix on later turns.
